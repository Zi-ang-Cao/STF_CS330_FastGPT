...
class LMEnv(gym.Env):
    ...
    def step(self, action):
        ...
        if not self.args.independent_reward:
            reward = reward.mean()
            done = torch.all(done)
            truncated = done.bool()
...
parser.add_argument("--total_timesteps", default=2E5, type=int)
#1.5E5

parser.add_argument("--data_items", default="127,733", type=str)
...
parser.add_argument("--sampling_mode", default="argmax", type=str)
...
parser.add_argument("--independent_reward", action='store_true', default=False)
...
class MyVecEnv(VecEnv):
    ...
    def step_without_reset(self):
        obs, rewards, terminated, truncated, buf_infos = self.env.step(
            torch.tensor(self.actions).bool().to(self.env.args.env_device)
        )
        if not self.env.args.independent_reward:
            self.buf_rews = buf_infos["last_reward"]
            self.buf_dones = buf_infos["last_done"]
        else:
                ...
                if k == "episode":
                    ...
                    for kk, vv in v.items():
                        if isinstance(vv, torch.Tensor):
                            buf_infos_i_episode[kk] = vv[i].cpu().numpy()
                        else:
                            buf_infos_i_episode[kk] = vv
                    buf_infos_i[k] = buf_infos_i_episode
                else:
                    buf_infos_i[k] = v[i]
            self.buf_infos[i] = buf_infos_i
...
def init_env_for_agent_training(args):
    lm_env = LMEnv(args=args)
    env = CustomMonitor(lm_env)
...
if __name__ == "__main__":
    ...
    if args.algorithm=="PPO":
        ...
        model.learn(total_timesteps=args.total_timesteps, 
                    ...
                    callback=CallbackList([checkpoint_callback, cust_callback]))
        if args.save:
