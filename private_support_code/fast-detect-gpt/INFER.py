...
class LMEnv(gym.Env):
    ...
    # def __init__(self, args, sampling_mode: str = "likelihood", topK_logistics: int=10, dataset: str="xsum", n_train:int = 1, 
    ...
    def step(self, action):
        ...
        if done:
            ...
            final_Raw_output = self.get_text()
        ...
        info = {"TimeLimit.truncated": False, 
                ...
                "final_RL_output": final_RL_output,
...
if __name__ == "__main__":
    ...
    parser.add_argument("--Infer_postFix", default="model_checkpoints/rl_model_150000_steps.zip", type=str)
    parser.add_argument("--Infer_folder", default="4070Ti", type=str)
    parser.add_argument("--Infer_modelName", default="L50_CT_S_0_RP_False_SC_True_SPC_True_1204_173343_1", type=str)
    ...
    ############################################
    assert args.num_env==1, "assert args.num_env==1"
    vec_env = init_env_for_agent_training(args, n_envs=args.num_env)



    Infer_model_path = os.path.join("tensorboard_Infer", args.Infer_folder, args.Infer_modelName, args.Infer_postFix)

    Infer_txt_path = os.path.join("tensorboard_Infer", "infer_result", f"{args.Infer_modelName}_{args.Infer_folder}_OUTPUT.txt")
    ...
    if args.algorithm=="PPO":
        ...
        if args.retrain_from != "":
            ...
            model = model.load(Infer_model_path)
            print("success!!!")
        ...
        RL_Text = info[0]["final_RL_output"]
        # RL_Text = vec_env.env_method("get_text")
        RL_Score = detector.infer(RL_Text)
        text_result += f"[RL]\n \tRL_Score={RL_Score}\n \tRL_Text={RL_Text}"
        text_result += "\n -------------------- \n"
        
        Raw_Text = info[0]["final_Raw_output"]
        Raw_Score = detector.infer(Raw_Text)
        text_result += f"[Raw]\n \tRaw_Score={Raw_Score}\n \tRaw_Text={Raw_Text}"
        text_result += "\n -------------------- \n"
        ...
        with open(f"{Infer_txt_path}", 'w') as file:
